...
basetabfunc <- function(data) {
  ...
  tab <- left_join(tab,
    ...
    mutate(
      ...
      No = if_else(Variable %in% c("Dapagliflozin", "Empagliflozin", "  Baseline-14 days", "  4 months-<Baseline"), "-", No),
    )

  # footnotes
  tab <- tab %>%
    mutate(
      footnote1 = if_else(!is.na(var) & var %in% modvars_rs, footnote_marker_alphabet(1, format = "latex"), "")
    ) %>%
